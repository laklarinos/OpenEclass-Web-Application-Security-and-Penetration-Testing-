...
if(isset($forumgo)) {
	...
	if ($result and mysql_num_rows($result) > 0) {
		...
		<td><input type=text name=forum_name size=40 class=\"FormData_InputText\">
		$token
		</td>
		</tr>
		<tr>
		...
		$tool_content .= "
  		<form action='$_SERVER[PHP_SELF]?forumcatsave=yes' method=post onsubmit=\"return checkrequired(this,'cat_title');\">
    		...
    		<tr>
      		<th class='left'>$langCat</th>
      		<td>
			<input type=text name=cat_title size=55 value='$cat_title' class=\"FormData_InputText\">
			$token
			</td>
    		</tr>
    		<tr>
	...
	elseif(isset($forumcatdel)) {	
		...
		if (!empty($_GET['delete_token']) || $_SESSION['delete_token'] !== $_GET['delete_token'] ) {
			...
			$tool_content .= "<a href='forum_admin.php'>$langBack</a></p>";
			$tool_content .= "<p>&nbsp;</p>";
			draw($tool_content, '1', 'forum_admin', $head_content);
	...
	elseif(isset($forumgodel)) {
		...
		while(list($cat_id, $cat_title) = mysql_fetch_row($result)) {
			...
			<a href='$_SERVER[PHP_SELF]?forumcatdel=yes&cat_id=$cat_id&ok=0&delete_token=".$_SESSION['delete_token']."' onClick='return confirmation();'>
			<img src='../../template/classic/img/delete.gif' border='0' title='$langDelete'></img></a>

			<a href='$_SERVER[PHP_SELF]?forumcatnotify=$link_notify&cat_id=$cat_idd'>	
		...
		<th class='left'>$langCat</th>
		<td>
		<input type=text name=catagories size=50 class='FormData_InputText'>
		</td>
		</tr>
		<tr><th>&nbsp;</th>
		<td>
