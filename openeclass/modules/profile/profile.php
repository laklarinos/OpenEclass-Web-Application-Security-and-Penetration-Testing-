...
if (isset($submit) && (!isset($ldap_submit)) && !isset($changePass)) {
	...
	else if(isset($user_exist) AND ($username_form==$user_exist) AND ($username_form!=$uname)) {
		header("location:". $_SERVER['PHP_SELF']."?msg=5");
		exit();
	}

	// check if email is valid
	else if (!email_seems_valid($email_form) and check_prof()) {
		header("location:". $_SERVER['PHP_SELF']."?msg=6");
		exit(); 
	}
	
	else if (!isset($_POST['CSRFName']) or !isset($_POST['CSRFToken'])){
		// csrftoken is not set
		header("location:". $_SERVER['PHP_SELF']."?msg=7");
		exit();
	}
	
	else if (!validateCsrfToken($_POST['CSRFName'], $_POST['CSRFToken'])){
		// csrf is not valid
		header("location:". $_SERVER['PHP_SELF']."?msg=7");
		exit();
	}
		...
		// 	$tool_content .= "<p class='caution_small'>$langError<br />";
...
if ((!isset($changePass)) || isset($_POST['submit'])) {
	...
	if (isset($_SESSION['shib_user'])) {
		...
		$tool_content .= "<td>
		<input class='FormData_InputText' type=\"text\" size=\"40\" name=\"prenom_form\" value=\"$prenom_form\">
		$token
		</td>";
	}
